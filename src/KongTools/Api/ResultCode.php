...
use KongTools\Utils\Json\JsonUtil;
...
class ResultCode{
     ...
     * @return mixed|string
     */
    public static function codeMapMessage($code){
        $map = [
            self::SUCCESS => 'ok',
            self::ERROR => 'error',
     ...
     * @return array
     */
    public static function jsonPrettyOption(){
        return [
            'json_pretty' => true,
        ];
    }
     ...
     * @param array $options
     */
    public static function success($msg='ok', $data=[], $options=[]){
        header('Content-Type:application/json; charset=utf-8');
        $ret = [
            'status' => 1,
            'code' => self::SUCCESS,
            'msg'  => self::codeMapMessage(self::SUCCESS),
            'data' => $data,
            'time' => date('YmdHis',time()),
        ];
        $json = json_encode($ret,JSON_UNESCAPED_UNICODE);

        if(isset($options['json_pretty']) && $options['json_pretty']==true){
            echo JsonUtil::jsonPretty($json,255);die;
        }else{
            echo $json;die;
        }
    }
     ...
     * @param array $options
     */
    public static function error($msg='', $data=[], $options=[]){
        header('Content-Type:application/json; charset=utf-8');

        $msg = empty($msg) ? self::codeMapMessage(self::ERROR) : $msg;
     ...
     * @param array $options
     */
    public static function errorWithCode($code, $msg='', $data=[], $options=[]){
        header('Content-Type:application/json; charset=utf-8');

        $msg = empty($msg) ? self::codeMapMessage($code) : $msg;
